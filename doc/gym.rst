...
.. autoattribute:: pykep.trajopt.gym.cassini1a

This is the same MGA problem as :attribute:`~pykep.trajopt.gym.cassini1`, but with using
...
.. autoattribute:: pykep.trajopt.gym.cassini1n
...
It is fundamentally similar to :attribute:`~pykep.trajopt.gym.cassini1`, but it allows for one deep space maneuver (DSM) at any point
...
.. autoattribute:: pykep.trajopt.gym.eve_mga_1dsm
...
.. autoattribute:: pykep.trajopt.gym.eve_mga_1dsm_a

This is the same MGA-1DSM problem as :attribute:`~pykep.trajopt.gym.eve_mga_1dsm`, but with using
a different encoding of the decision variables. In this case, the decision variables are no longer directly
the times of flight but an equivalent set called the alpha encoding (see :attribute:`~pykep.trajopt.gym.cassini1a`).

.. autoattribute:: pykep.trajopt.gym.eve_mga_1dsm_n

This is the same MGA-1DSM problem as :attribute:`~pykep.trajopt.gym.eve_mga_1dsm`, but with using
a different encoding of the decision variables. In this case, the decision variables are no longer directly
the times of flight but an equivalent set called the eta encoding (see :attribute:`~pykep.trajopt.gym.cassini1n`).
...
This is the same MGA-1DSM problem as :attribute:`~pykep.trajopt.gym.juice`, but with using
the alpha encoding for the time of flight variables (see :attribute:`~pykep.trajopt.gym.cassini1a`).
