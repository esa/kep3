...
#define kep3_S11N_PLANET_EXPORT_KEY(PLA)                                                                               \
    ...
    BOOST_CLASS_TRACKING(kep3::detail::planet_inner<PLA>, boost::serialization::track_never)

#define kep3_S11N_PLANET_IMPLEMENT(PLA) BOOST_CLASS_EXPORT_IMPLEMENT(kep3::detail::planet_inner<PLA>)

#define kep3_S11N_PLANET_EXPORT(PLA) kep3_S11N_PLANET_EXPORT_KEY(PLA) kep3_S11N_PLANET_IMPLEMENT(PLA)
...
#define UDPLA_HAS_GET(NAME)                                                                                            \
    template <typename T>                                                                                              \
    using udpla_get_##NAME##_t = decltype(std::declval<std::add_lvalue_reference_t<const T>>().get_##NAME());          \
    template <typename T>                                                                                              \
    inline constexpr bool udpla_has_get_##NAME##_v = std::is_same_v<detected_t<udpla_get_##NAME##_t, T>, double>
...
// method
template <typename T>
using udpla_eph_t = decltype(std::declval<std::add_lvalue_reference_t<const T>>().eph(std::declval<const epoch &>()));
...
template <typename T>
inline constexpr bool udpla_has_get_name_v = std::is_same_v<detected_t<udpla_get_name_t, T>, std::string>;
...
template <typename T>
inline constexpr bool udpla_has_get_extra_info_v = std::is_same_v<detected_t<udpla_get_extra_info_t, T>, std::string>;

// udpla_has_period_v<T> is True if T has the method:
// double period(const epoch&)
template <typename T>
using udpla_period_t
    = decltype(std::declval<std::add_lvalue_reference_t<const T>>().period(std::declval<const epoch &>()));
template <typename T>
inline constexpr bool udpla_has_period_v = std::is_same_v<detected_t<udpla_period_t, T>, double>;
...
struct kep3_DLL_PUBLIC_INLINE_CLASS planet_inner final : planet_inner_base {
    ...
    [[nodiscard]] double period(const kep3::epoch &ep) const final
    {
        ...
        if constexpr (udpla_has_period_v<T>) {
            ...
            throw not_implemented_error("A period nor a central body mu has been declared for '" + get_name()
...
BOOST_CLASS_TRACKING(kep3::detail::planet_inner_base, boost::serialization::track_never)
...
class kep3_DLL_PUBLIC planet
{
    ...
    using generic_ctor_enabler
        = std::enable_if_t<std::conjunction_v<std::negation<std::is_same<planet, detail::uncvref_t<T>>>,
...
public:
    ...
    explicit planet(T &&x) : m_ptr(std::make_unique<detail::planet_inner<detail::uncvref_t<T>>>(std::forward<T>(x)))
    {
    }
    // Copy constructor
    planet(const planet &);
