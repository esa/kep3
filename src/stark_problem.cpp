...
std::array<double, 7> stark_problem::propagate(const std::array<double, 7> &rvm_state, std::array<double, 3> thrust, double tof)
{
    ...
    if (std::get<0>(out) != taylor_outcome::time_limit) {
        throw std::domain_error("stark_problem: failiure to reach the final time requested during a propagation.");
    }
...
double stark_problem::get_mu() const
{
    return m_mu;
}
double stark_problem::get_veff() const
{
    return m_veff;
}
double stark_problem::get_tol() const
{
    return m_tol;
}
heyoka::taylor_adaptive<double> stark_problem::get_ta() const
{
    return m_ta;
}
heyoka::taylor_adaptive<double> stark_problem::get_ta_var() const
{
    return m_ta_var;
}

// Setters
void stark_problem::set_mu(double mu_in)
{
    m_mu = mu_in;
    *(m_ta.get_pars_data()) = m_mu;
    *(m_ta_var.get_pars_data()) = m_mu;
};
void stark_problem::set_veff(double veff_in)
{
    m_veff = veff_in;
    *(m_ta.get_pars_data()+1) = m_veff;
    *(m_ta_var.get_pars_data()+1) = m_veff;
